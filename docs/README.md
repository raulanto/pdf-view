...
PDF View es un visor PDF local para Arch Linux y Omarchy. La interfaz está hecha en Quickshell/QML y sus servicios en Rust. Permite leer, buscar, seleccionar y copiar texto, navegar por miniaturas e índice y usar OCR local, con notas y subrayados guardados dentro del PDF.
...
El visor trabaja con archivos locales y permite guardar notas y subrayados en el PDF. Zoom, rotación y OCR no guardan cambios en el PDF. No incluye edición del contenido de las páginas, firmas, cuentas ni sincronización remota. PDFium, Poppler, Cairo, Tesseract y Quickshell/Qt siguen siendo bibliotecas externas; el backend propio está escrito en Rust.
